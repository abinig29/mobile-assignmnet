...
class FoundItem extends Equatable {
  ...
  Map<String, dynamic> toMap() {
   ...
   Map<String, dynamic> toSqlMap() {
    ...
    result.addAll({'location': location});
    result.addAll({'userAddress': jsonEncode(userAddress) });
    if(claimQuestions != null){
      result.addAll({'claimQuestions': jsonEncode(claimQuestions)});
    }
    if(claimQuestionAnswers != null){
      result.addAll({'claimQuestionAnswers':jsonEncode(claimQuestionAnswers)});
    }
  ...
  factory FoundItem.fromMap(Map<String, dynamic> map) {
    return FoundItem(
      ...
      claimQuestions: map['claimQuestions'] != null ? List<Map<String, dynamic>>.from(map['claimQuestions']?.map((x) => Map<String, dynamic>.from(x))) : null,
      claimQuestionAnswers: List<String>.from(map['claimQuestionAnswers']),
    );
  }
    factory FoundItem.fromSqlMap(Map<String, dynamic> map) {
    return FoundItem(
      id: map['id'],
      title: map['title'] ?? '',
      description: map['description'] ?? '',
      ...
      userPicturePath: map['userPicturePath'],
      location: map['location'] ?? '',
      userAddress:jsonDecode(map['userAddress']) ??"",
      claimQuestions: map['claimQuestions'] != null ? jsonDecode(map['claimQuestions']?.map((x) => Map<String, dynamic>.from(x)))  : null,
      claimQuestionAnswers: jsonDecode(map['claimQuestionAnswers']),
    );
  }

  String toJson() => json.encode(toMap());

  factory FoundItem.fromJson(String source) => FoundItem.fromMap(json.decode(source));
}
